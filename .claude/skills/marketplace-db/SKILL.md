...
- **Write `(select auth.uid())`, never bare `auth.uid()`, inside a policy.** Bare, it is re-evaluated **per row**; wrapped in a subselect Postgres hoists it into an InitPlan evaluated once per query. Same for `private.is_admin()` and `private.owns_order()`. On a catalogue scan that is one function call versus one per listing, and it is invisible until you look at a plan.
- **One permissive policy per (role, command) per table.** Postgres evaluates every permissive policy for every row and ORs the results, so a user-facing SELECT policy sitting alongside an admin `for all` policy doubles the work on every read for nothing — the read policy already admits admins. Admin policies are therefore scoped to `insert` / `update` / `delete` as three single-command policies, since Postgres has no `for insert, update, delete`. More policy objects, strictly fewer evaluations.
